...
u8_t buf_read[BLOCK_BUFFER_SIZE];
			 ...
			 u8_t *data, int len)
{
	...
	if (len > BLOCK_BUFFER_SIZE) {
		SYS_LOG_ERR("buffer length too large (%d)", len);
		return false;
	}

	memset(buf_read, 0x00, BLOCK_BUFFER_SIZE);
	rc = flash_read(dev, offset, buf_read, len);
	if (rc) {
